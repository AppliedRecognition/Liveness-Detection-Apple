...
class ImageIterator: IteratorProtocol {
    
    typealias Element = (URL, Bool, UIImage)
    ...
    convenience init() {
        self.init(urls: ImageIterator.spoof.compactMap({
            if let url = URL(string: $0) {
                return (true, url)
            }
            return nil
        }) + ImageIterator.live.compactMap({
            if let url = URL(string: $0) {
                return (false, url)
            }
            return nil
    ...
    func next() -> (URL, Bool, UIImage)? {
        self.index += 1
        if self.index >= urls.count {
            return nil
        }
        while self.index < urls.count {
            if let url = try? self.localURL(of: urls[self.index].1), let data = try? Data(contentsOf: url), let image = UIImage(data: data) {
                return (url, urls[self.index].0, image)
            }
            self.index += 1
        }
        return nil
    ...
    private func localURL(of url: URL) throws -> URL {
        let localURL = try self.cacheURL(of: url)
    ...
    private func cacheURL(of url: URL) throws -> URL {
