...
/// - Since: 1.0.0
public struct DetectedSpoofDevice: Codable {
    ...
    /// - Since: 1.0.0
    public func flipped(imageSize: CGSize) -> DetectedSpoofDevice {
        let transform = CGAffineTransform(scaleX: -1, y: 1).concatenating(CGAffineTransform(translationX: imageSize.width, y: 0))
        return DetectedSpoofDevice(boundingBox: self.boundingBox.applying(transform), confidence: self.confidence)
    }
}
